...
// Note: The hash entry struct is defined but not used by the provided functions.
...
// LS_Status LS_ACQUIRE(void* l, bool reentrant, LockFunc&& lock_fn, Args&&... args) {
    ...
    if (!entry) {
        // Use std::invoke to call the lock function
        std::invoke(std::forward<LockFunc>(fn), l, std::forward<Args>(args)...);
...
// LS_Status LS_RELEASE(void* l, bool reentrant, UnlockFunc&& unlock_fn, Args&&... args) {
    ...
    if (reentrant) {
        ...
        if (val == 0) {
            // Use std::invoke to call the unlock function
            std::invoke(std::forward<UnlockFunc>(fn), l, std::forward<Args>(args)...);
            return LS_Status::LS_RELEASE_NOW;
        }
        return LS_Status::LS_SKIP_RELEASE;
    }
    // Use std::invoke to call the unlock function for the non-reentrant case
    std::invoke(std::forward<UnlockFunc>(fn), l, std::forward<Args>(args)...);
    DecrementRef(l, entry);
    return LS_Status::LS_RELEASE_NOW;
}

#endif // SHIELDING_INVOKE_H